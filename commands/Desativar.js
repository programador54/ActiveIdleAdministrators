...
module.exports.run = async (client, message, args) => {
	if (message.channel.id === '773647449843171328') {
		message.delete();
		if (message.content.toLowerCase().includes(args[0])) return message.channel.send('Não escreva nada junto ao comando!').then(msg => msg.delete({timeout: 27000}))
		...
		if (10 > verificar) {
			message.channel.send(`<a:umL:775067435010228235> **|** Qual é o ID do seu Bot?`).then(msg => {
				let cp = message.channel
					...
					.on('collect', a => {
						...
						message.channel.send(`<a:doisL:775067480489852960> **|** Qual é a prefix do seu bot?`).then(async msg2 => {
							msg.delete();
							a.delete();
							...
							let cl = message.channel
								...
								.on('collect', async d => {
									...
									message.channel
										.send('<a:tresL:775067534919467030> **|** Qual a biblioteca? `(dbd, js, dbs, py, dbm)`')
										.then(async msg3 => {
											d.delete();
											msg2.delete();
											msg3.delete({ timeout: 10000 });
											...
											let ck = message.channel
												...
												.on('collect', async c => {
													...
													await db.set(
														`prefix_${message.guild.id}_${message.author.id}`,
														desc
													);
													await db.set(`ID_${message.author.id}`, id);
